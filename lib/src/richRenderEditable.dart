...
typedef RichSelectionChangedHandler = void Function(
    TextSelection selection, RichRenderEditable renderObject, SelectionChangedCause cause);
...
class RichRenderEditable extends RenderBox {
  ...
  RichRenderEditable({
    ...
    this.supportRichText,
        ...
        _handleRichText = hasSpecialText(text),
        _textPainter = TextPainter(
          ...
          locale: locale,
//          strutStyle:
//              supportRichText && hasSpecialText(text) ? null : strutStyle,
        ),
  ...
  void _updateSelectionExtentsVisibility(Offset effectiveOffset, TextSelection selection) {
  ...
  void _updateVisibleRegionMinY() {
    ...
    List<TextBox> boxs = _textPainter.getBoxesForSelection(textSelectionDelegate.textEditingValue.selection
        .copyWith(baseOffset: 0, extentOffset: _textPainter.text.toPlainText().length));
    boxs.forEach((f) {
  ...
  static const int _kControlMask =
      1 << 12; // https://developer.android.com/reference/android/view/KeyEvent.html#META_CTRL_ON
  ...
  int _handleVerticalArrows(bool upArrow, bool downArrow, bool shift, int newOffset) {
    ...
    final Offset caretOffset =
        _textPainter.getOffsetForCaret(TextPosition(offset: _extentOffset), _caretPrototype);
      ...
      case _kXKeyCode:
        if (!selection.isCollapsed) {
          ...
          textSelectionDelegate.textEditingValue = TextEditingValue(
            text: selection.textBefore(text.text) + selection.textAfter(text.text),
  ...
  void _handleDelete() {
    if (selection.textAfter(text.text).isNotEmpty) {
      textSelectionDelegate.textEditingValue = TextEditingValue(
        text: selection.textBefore(text.text) + selection.textAfter(text.text).substring(1),
        selection: TextSelection.collapsed(offset: selection.start),
      );
    } else {
      textSelectionDelegate.textEditingValue = TextEditingValue(
  ...
  set text(TextSpan value) {
    ...
    _handleRichText = hasSpecialText(value);
    if (handleRichText) {
...
//    }
  ...
  void describeSemanticsConfiguration(SemanticsConfiguration config) {
    ...
    config
      ..value = obscureText ? obscuringCharacter * text.toPlainText().length : text.toPlainText()
  ...
  List<TextSelectionPoint> getEndpointsForSelection(TextSelection selection) {
    ...
    ///zmt
    final Offset effectiveOffset = (_initialOffset ?? Offset.zero) + _paintOffset;
...
//          _textPainter.getOffsetForCaret(temp.extent, _caretPrototype);
      ...
      final Offset start = Offset(0.0, preferredLineHeight) +
          _getCaretOffset(
              effectiveOffset,
              TextPosition(offset: temp.baseOffset, affinity: selection.affinity),
      ...
      final Offset end = Offset(0.0, preferredLineHeight) +
          _getCaretOffset(
              ...
              TextPosition(offset: selection.extentOffset, affinity: selection.affinity));
  ...
  Offset _getCaretOffset(Offset effectiveOffset, TextPosition textPosition, TextPosition textInputPosition) {
    final Offset caretOffset =
        _textPainter.getOffsetForCaret(textPosition, _caretPrototype) + effectiveOffset;
    return caretOffset;
  }
  ...
  void selectPositionAt({@required Offset from, Offset to, @required SelectionChangedCause cause}) {
    ...
    if (onSelectionChanged != null) {
      ...
      //zmt
      if (handleRichText) {
        fromPosition = convertTextPainterPostionToTextInputPostion(text, fromPosition);
        toPosition = convertTextPainterPostionToTextInputPostion(text, toPosition);
      }
  ...
  void selectWordsInRange({@required Offset from, Offset to, @required SelectionChangedCause cause}) {
    ...
    if (onSelectionChanged != null) {
      ...
      final TextSelection lastWord = to == null
          ? firstWord
          : _selectWordAtOffset(_textPainter.getPositionForOffset(globalToLocal(to - _paintOffset)));
  ...
  void selectWordEdge({@required SelectionChangedCause cause}) {
    ...
    if (onSelectionChanged != null) {
      final TextPosition position =
          _textPainter.getPositionForOffset(globalToLocal(_lastTapDownPosition - _paintOffset));
      ...
      if (position.offset - word.start <= 1) {
        selection = handleRichText
            ? convertTextPainterSelectionToTextInputSelection(
                text, TextSelection.collapsed(offset: word.start, affinity: TextAffinity.downstream))
            : TextSelection.collapsed(offset: word.start, affinity: TextAffinity.downstream);
      } else {
        selection = handleRichText
            ? convertTextPainterSelectionToTextInputSelection(
                text, TextSelection.collapsed(offset: word.end, affinity: TextAffinity.upstream))
  ...
  TextSelection _selectWordAtOffset(TextPosition position) {
    ...
    return handleRichText ? convertTextPainterSelectionToTextInputSelection(text, selection) : selection;
//    if (position.offset >= word.end)
...
//    return TextSelection(baseOffset: word.start, extentOffset: word.end);
    ...
    switch (defaultTargetPlatform) {
      ...
      default:
        return Rect.fromLTWH(
            0.0, _kCaretHeightOffset, cursorWidth, preferredLineHeight - 2.0 * _kCaretHeightOffset);
    }
  }
  ...
  void _paintContents(PaintingContext context, Offset offset) {
    ...
    ///zmt
    var actualSelection =
        handleRichText ? convertTextInputSelectionToTextPainterSelection(text, _selection) : _selection;

    if (actualSelection != null && !_floatingCursorOn) {
      if (actualSelection.isCollapsed && _showCursor.value && cursorColor != null)
        showCaret = true;
      else if (!actualSelection.isCollapsed && _selectionColor != null) showSelection = true;
    ...
    if (showCaret) _paintCaret(context.canvas, effectiveOffset, actualSelection.extent, _selection.extent);
  ...
  void _paintCaret(
      Canvas canvas, Offset effectiveOffset, TextPosition textPosition, TextPosition textInputPosition) {
    assert(_textLayoutLastWidth == constraints.maxWidth);
    ...
    final Paint paint = Paint()..color = _floatingCursorOn ? backgroundCursorColor : _cursorColor;

    final Offset caretOffset = _getCaretOffset(effectiveOffset, textPosition, textInputPosition);
    ...
    if (defaultTargetPlatform != TargetPlatform.iOS &&
        _textPainter.getFullHeightForCaret(textPosition, _caretPrototype) != null) {
  ...
  void _paintRichText(PaintingContext context, Offset offset) {
    if (!handleRichText) return;
    ...
    _paintRichTextChildren(text.children, canvas, rect);
    canvas.restore();
  }

  void _paintRichTextChildren(List<TextSpan> textSpans, Canvas canvas, Rect rect, {int textOffset: 0}) {
